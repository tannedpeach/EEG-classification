...
# Cross validation inspired in https://www.machinecurve.com/index.php/2020/02/18/how-to-use-k-fold-cross-validation-with-keras/
...
def train_model(hparams, logs_dir, le, weights, class_probs=None):
    ...
    # Train the model
    model.fit(train_dataset, epochs=EPOCHS, steps_per_epoch=np.ceil(2200 / BATCH_SIZE),
                        validation_data=val_dataset, class_weight=weights, callbacks=[tboard_callback])
...
#     seizure_folds1 = pickle.load(open(cross_val_file_2, "rb"))
#     test_fold1 = seizure_folds1[-1]
#     # test_dataset1 = get_test_dataset(data_dir, test_fold1, le)
#     print("[INFO] evaluating network...")
#     scores = model.evaluate(test_fold1, verbose=0)
#
#     with tf.summary.create_file_writer(logs_dir + '/run').as_default():
#         tf.summary.scalar(model.metrics_names[5], scores[5], step=1)
#         tf.summary.scalar(model.metrics_names[6], scores[6], step=1)
...
#     print("[INFO] Predicting network and creating confusion matrix...")
#
#     test_pred = np.argmax(model.predict(test_fold1), axis=1)
#     save_confusion_matrix(test_labels, test_pred, le.classes_, logs_dir)
#
#
#
